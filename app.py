...
# --- SZIGORÍTOTT HATÁRVONAL (A vizualizációhoz is) ---
HU_COORDS = [
...
def fetch_data_v3(date, weights):
    ...
    # API HÍVÁS DARABOLÁSA (Chunking), hogy ne legyen JSON hiba
    chunk_size = 25 
...
if st.sidebar.button("Hard Reset (Cache törlés)"):
    st.cache_data.clear()
    st.rerun()

target_date = st.sidebar.date_input("Dátum", datetime.now() + timedelta(days=1))
weights = get_weights_v3()

with st.spinner('Adatok lekérése (biztonságos módban)...'):
    df = fetch_data_v3(target_date, weights)
    
    if not df.empty:
        # Szélsőértékek és helyszínek
        ...
        c1, c2 = st.columns(2)
        c1.metric("Belföldi MIN", f"{round(min_row['min'], 1)} °C", f"{min_city} környéke")
        c2.metric("Belföldi MAX", f"{round(max_row['max'], 1)} °C", f"{max_city} környéke")
        
        st.success(f"📍 **Minimum:** {min_city} ({round(min_row['min'],1)}°C) | **Maximum:** {max_city} ({round(max_row['max'],1)}°C)")

        st.divider()
        m1, m2 = st.columns(2)

        def draw_map_v3(data, col, colors, title):
            fig = px.scatter_mapbox(data, lat="lat", lon="lon", color=col, 
                                    color_continuous_scale=colors, zoom=6.0,
        ...
        m1.plotly_chart(draw_map_v3(df, "min", "Viridis", "Minimum hőmérsékletek"), use_container_width=True)
        m2.plotly_chart(draw_map_v3(df, "max", "Reds", "Maximum hőmérsékletek"), use_container_width=True)
    else:
        st.error("Hiba történt az adatok lekérésekor. Próbáld meg a Hard Reset gombot!")
