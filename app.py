...
st.markdown("""
    ...
    .tech-details { background-color: #f8f9fa; padding: 20px; border-radius: 10px; font-size: 0.9rem; border-left: 5px solid #0d6efd; color: #333; }
...
@st.cache_data
def load_all_towns():
    # Itt a teljes 3155-ös lista helye (példa adatokkal)
    return [
...
with side_c:
    ...
    # Tiszta Markdown technikai leírás (HTML mentesen, korábbi részletes D-MOS-sal)
    st.markdown(f"""
    <div class="tech-details">
    
    **1. DINAMIKUS SÚLYOZÁS (D-MOS)** A rendszer nem statikus súlyokat használ. Minden futtatáskor lekéri az elmúlt 24 óra **ténylegesen mért** (METAR/Archive) adatait és összeveti azokat a modellek korábbi jóslataival. A súlyozás az inverz MAE (Mean Absolute Error) alapján dől el: amelyik modell tegnap pontosabb volt, az ma nagyobb befolyással bír.
    ...
    **3. TELEPÜLÉSSZINTŰ ELEMZÉS** A rendszer képes Magyarország mind a **3155 településének** egyedi koordinátájára számítást végezni. A hatékonyság érdekében *Batch Processing* eljárást használunk: az adatokat 50-es csoportokban kérjük le, így a futási idő drasztikusan lecsökken.
